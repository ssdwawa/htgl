enterpriseApp.controller('eJobApplyRecordController',['$scope', 'commonService','jobService','$state',
    function ($scope, commonService,jobService,$state) {
        ...
        var loadTableData = function (data) {
            jobService.getJobApplyRecord(_.extend({choseTime: $scope.choseTime}, data), 1).then(function (res) {
        ...
        $scope.checkResume = function (row) {
            $state.go('enterprise.check_resume', {id:row.id});
        }
    }]);