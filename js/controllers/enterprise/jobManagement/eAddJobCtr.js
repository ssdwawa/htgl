enterpriseApp.controller('eAddJobCtr', ['$scope', 'commonService', 'jobService', '$state', '$stateParams',
    'platformService', '$timeout', '$layer',
    function ($scope, commonService, jobService, $state, $stateParams, platformService, $timeout,$layer) {
        'use strict';
        ...
        if ($stateParams.id) {
            jobService.getJobInfo({id: $stateParams.id},1).then(function (res) {
                $scope.formData = res;
                ...
                jobService.isJobApply({id: $stateParams.id}, 1).then(function (res) {
                    if(res != '0'){
                        $scope.isFormDisabled = true;
                    }
                },function (err) {
        ...
        var submitData = function () {
            ...
            $scope.formData.companyName = $scope.$parent.enterInfo.name;
            jobService.submitJob($scope.formData, action,1).then(function (res) {
                if (res.retCode === '-1') {
                    ...
                    $state.go('enterprise.e_publish_job');
                }
            }, function (err) {
            });
        };
