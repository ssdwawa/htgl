enterpriseApp.controller('ePublishJobController',['$scope', 'commonService', 'jobService', '$state', '$layer','$timeout',
    function ($scope, commonService, jobService,$state,$layer,$timeout) {
        ...
        var loadTableData = function (data) {
            jobService.getJobList(data,1).then(function (res) {
        ...
        $scope.edit = function (row) {
            $state.go('enterprise.e_add_job', {id: row.id});
           /* if(row.status == '1'){
        ...
        var changeStatus = function (id,action) {
            jobService.changeJobStatus({id: id, status: action},1).then(function (res) {
                if (res.retCode === '0') {
                    var index = $layer.open({
                        type: 'alert',
                        layerOption: {
        ...
        $scope.changeJobStatus = function (row, action) {
            if(action == '0'){
                jobService.isJobApply({id: row.id}, 1).then(function (res) {
                    if(res != '0'){
